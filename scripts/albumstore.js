...
AlbumStore.getSub = function(year, month, day, subdir, options, callback) {
    if (!year) throw 'no year';
    if (!month) throw 'no month';
    if (!day) throw 'no day';
    if (!subdir) throw 'no subdir';
    if (!options) throw 'no options';

    var path = year + '/' + month + '-' + day + '/' + subdir;
    ...
    try {
        ...
        // I should do something with the data I retrieve
    }
    catch(e) {
        // if it's a file not found error
        if (e.code === 'ENOENT') {
            console.log('\talbum has no JSON on disk: %s', path);
            ...
            if (isDynamic) {
                ...
                http.get(url, function (res) {
                    ...
                    res.on('end', function () {
                        ...
                        // I should call the callback here
                    });
